...
{
    if (strlen(CONFIG_NTFY_TOPIC) == 0) {
        ESP_LOGD(TAG, "topic leeg — geen push");
        return ESP_OK;
    }

    char url[256];
    const int n = snprintf(url, sizeof(url), "%s/%s", CONFIG_NTFY_SERVER, CONFIG_NTFY_TOPIC);
...
{
#if CONFIG_NTFY_CLIENT_ENABLE
    ESP_LOGI(TAG, "M-011a: ntfy_client actief (server=%s, topic=%s)", CONFIG_NTFY_SERVER,
             (CONFIG_NTFY_TOPIC[0] != '\0') ? "ingesteld" : "leeg");
#else
