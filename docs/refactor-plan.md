...
- **`PLATFORM_ESP32S3_4848S040`:** niet meer een **actief doelplatform** voor refactors (zelfde prioriteit als CYD/TTGO: niet actief gebruikt); blijft **in de codebase**; op termijn uitfaseerbaar.
- **Legacy:** **CYD24 / CYD28 / TTGO** gefaseerd en **compile-time gecontroleerd** uitfaseren (`platform_config.h`), geen “big bang”.
...
- Correctheid datastroom ↔ UI (o.a. 2h, footer) voor de **JC3248-referentielijn** en andere **actief gebruikte** moderne ESP32-S3-platforms — **niet** gericht op **4848S040** (zie besluit §4).
...
- **4848-specifieke** refactors, parity-werk of feature-pariteit; **4848** valt **buiten actieve refactorscope** zolang die scope op JC3248/modern S3 gericht is.
...
| Kleine **bugfix** `ret_2h` ↔ `prices[3]` voor JC3248 via sync in `.ino` (zelfde patroon als CYD/4848) | **Toegepast in code** (`d5f258b`) — *hardware smoke-test: uitkomst niet in repo vastgelegd; zie WP4* |
| **`PLATFORM_ESP32S3_4848S040`** | Niet meer actief doelplatform; buiten actieve refactorscope; code blijft in tree; op termijn uitfaseerbaar samen met andere niet-prioritaire paden. |
...
| **Gedeelde 4-symbol / 2h-logica** | JC3248 deelt macro’s met o.a. LCDWIKI; **4848** blijft in code maar **buiten actieve refactor-scope** (zie §4). |
...
| **WP6** | Veilige verwijdering zuivere CYD/TTGO + later **4848**-uitfaseringspaden | `todo` | CYD/TTGO eerst; **4848** meenemen bij inventaris/when actief niet meer gebruikt; *geen impact JC3248*. |
...
- **Direct verwijderen** of opschonen van **`PLATFORM_ESP32S3_4848S040`**: nog niet; pas na inventaris en separaat besluit (code blijft staan).
...
| Datum | Notitie |
|-------|---------|
| 2026-03-21 | Scopebeslissing: **`PLATFORM_ESP32S3_4848S040`** niet meer actief doelplatform (zoals CYD/TTGO qua prioriteit); **buiten actieve refactorscope**; code blijft; op termijn uitfaseerbaar; **robuuste 2h-semantiekfix** richt zich op **moderne actieve S3** (o.a. JC3248), niet op 4848. |
